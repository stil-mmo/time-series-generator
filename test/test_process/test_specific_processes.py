...
@pytest.fixture
def process_list():
    process_list = ProcessStorage(GENERATOR_LINSPACE)
    return process_list
...
def test_random_walk():
    with initialize(version_base=None, config_path="../.."):
        cfg = compose(config_name="config")
        Process.cfg = cfg.process
        simple_random_walk = SimpleRandomWalk(GENERATOR_LINSPACE)
        ...
        for i in range(1, len(values)):
            ...
            assert isclose(diff, 1.0)  # Assuming fixed_walk is set to 1.5
