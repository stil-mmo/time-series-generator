...
class TestRandomWalkProcess(TestCase):
    def test_generate_parameters(self):
        up, down = RandomWalkProcess((0.0, 1)).generate_parameters()
        self.assertTrue(up + down == 1.0)

    def test_generate_init_values(self):
        init_value = RandomWalkProcess((-1.0, 1.0)).generate_init_values()
        self.assertTrue(-1 <= init_value[0] <= 1)

    def test_generate_time_series(self):
        rw_time_series, info = RandomWalkProcess((-1.0, 1.0)).generate_time_series(
            (10, (0.5, 0.5))
        )
